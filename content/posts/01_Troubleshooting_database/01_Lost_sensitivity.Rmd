---
title: "Lost Sensitivity"
author: "Lenny Lin"
date: "2022-08-16"
categories: Troubleshooting Database
tags: Fishbone Demo
toc: yes
weight: 1
lastmod: "2022-08-16"
---


<!--more-->

---

```{r setup, include=FALSE}
knitr::opts_chunk$set(collapse = TRUE)
```

```{r, warning = F, include=F}
#library(SixSigma)
library(qcc)

#font = c(1, 2, 3), values for the font to use. The values refer, in order, to branches, causes and effect.
# 1: normal, 2: bold, 3: italic
#cex = c(1, 0.8, 1)) # cex, values for the graphical character expansion. The values refer, in order, to branches, causes and effect
# they are the size.
```



```{r, echo = F }
cause.and.effect(cause=list('MS/MS' = c("ESI", "EMV", "Tuning Report"),
                            LC = c("Pump", "Flow", "Mobile Phase"),
                            "Human Errors" = c("Wrong vial"),
                            "Sample Preparation" = c("Lost in Elute"),
                            Methods = c("ratio 1", "ratio 2")),
                 effect = "Low Sensitivity",
                 title = "Low Sensitivity with LC-MS/MS",
                 font = c(1, 3, 2), 
                 cex = c(1, 0.7, 1))
                 

```


```{r, echp = F}
cause.and.effect(cause=list(Measurements=c("Micrometers", "Microscopes", "Inspectors"),
                            Materials=c("Alloys", "Lubricants", "Suppliers"),
                            Personnel=c("Shifts", "Supervisors", "Training", "Operators"),
                            Environment=c("Condensation", "Moisture"),
                            Methods=c("Brake", "Engager", "Angle"),
                            Machines=c("Speed", "Lathes", "Bits", "Sockets")),
                 effect="Surface Flaws",
                 title = "Low Sensitivity with LC-MS/MS",
                 font = c(1, 3, 2), 
                 cex = c(1, 0.7, 1))

```


[Reference](https://fenxihuaxue.netlify.app/docs/01-mass-spec-lc-ms/part-05-troubleshooting/02_lcmsms_clinical_troubleshooting/)

